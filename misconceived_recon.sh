...
subdomain_recon() {
    ...
    if [[ $(cat $subdomain_dir/final_live.txt) ]]; then
        ...
        my_diff $subdomain_dir/live_summary.txt $subdomain_dir/new_live_summary.txt "Live Summary" $subdomain_webhook
    fi
    ...
    if [[ $(cat "$subdomain_dir/new_dnsreaper-takeovers.json" 2> /dev/null) ]]; then
        ...
        send_to_discord "*No findings from DNSReaper subdomain takeover detection*" $subdomain_webhook
    fi
    ...
    if [[ $(cat "$subdomain_dir/new_nuclei_takeovers.txt" 2> /dev/null) ]]; then
        print_message "Report from:" "Nuclei (-tags takeovers)"
        cat "$subdomain_dir/nuclei_takeovers.txt"
        
        send_to_discord "Nuclei (-tags takeovers) report:" $subdomain_webhook "$subdomain_dir/nuclei_takeovers.txt"
